...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Mix texture with procedural result
    // Adjust blend_factor to control the influence of each layer
    let texture = textureSample(material_color_texture, material_color_sampler, uv);
    let blend_factor: f32 = 0.1;
    var final_color = mix(rez, texture.rgb, blend_factor);

    return vec4<f32>(final_color, texture.a);
}
